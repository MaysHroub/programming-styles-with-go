...
func mapConcurrent(splitFunc func(string, *chan []pair, *sync.WaitGroup), dataChunks []string) [][]pair {
	pairsLists := [][]pair{}
	mailbox := make(chan []pair)

	var wg sync.WaitGroup

	for _, chnk := range dataChunks {
		wg.Add(1)
		go splitFunc(chnk, &mailbox, &wg)
	}

	go func(wg *sync.WaitGroup, mailbox *chan []pair) {
		wg.Wait()
		close(*mailbox)
	}(&wg, &mailbox)

	for resultedPairs := range mailbox {
		pairsLists = append(pairsLists, resultedPairs)
	}
...
func split(text string, mailbox *chan []pair, wg *sync.WaitGroup) {
	defer wg.Done()
